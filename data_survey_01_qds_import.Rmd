...
v1_reorder <- v1 |> 
  select(
    ...
    qds_date_visit = TODAY_V1, 
    qds_visit_type = VISIT_TYPE, 
    qds_ii         = II_V1, 
    qds_visit      = VISIT_V1,
    ...
    mms_12 = MMS_11, 
    MMS_S, 
    # Calculated columns from SPSS
    mms_score, MMS_total, MMS_severity,
    ...
    # Calculated columns from SPSS
    weight_kg_V1, height_meter_V1, height_meter_sq_V1, BMI_V1, 
    ...
    srh_52i = S43_V1I,
    # Calculated columns from SPSS
    HS1_V1_dichotomous, Mental_Health_dx, pain, Substance_abuse_hs, 
    Substance_use_30d,
   
    # PTSD Screen
    PTSD1_V1, PTSD2_V1, PTSD3_V1, PTSD4_V1,
    # Calculated columns from SPSS
    PTSD_V1_total, PTSD_V1_postive,
    ...
    # Calculated columns from SPSS
    # Changing the name prefix to match the others
    BRS2_QD_dichot = BRIS2_QD_dichot,
    BRS4_QD_unintent_slp = QD_unintent_slp,
    ...
    # Changing the name prefix to match the others
    af_qf_v1 = AlcQF_V1,
    af_binge_drinker_v1 = binge_drinker_V1, 
    af_heavy_drinker_v1 = Heavy_drinker,
    ...
    # Calculated columns from SPSS
    # Changing the name prefix to match the others
    AQ_PA = PA, AQ_VA = VA, AQ_A = A, AQ_HA = HA, AQ_total = AQ,
   
    # CES-D
    CES1_V1, CES2_V1, CES3_V1, CES4_V1, CES5_V1, CES6_V1, CES7_V1, CES8_V1, 
    CES9_V1, CES10_V1,
    # Calculated columns from SPSS
    CES5_V1r, CES8_V1r, CES_V1_total, CES_V1_total_labelled, CES1_V1_dichot, 
    CES2_V1_dichot, CES3_V1_dichot, CES4_V1_dichot, CES5_V1r_dichot, 
    CES6_V1_dichot, CES7_V1_dichot, CES8_V1r_dichot, CES9_V1_dichot, 
    CES10_V1_dichot, CES_V1_dichot_total, CES_V1_dichot_labelled, 
   
    # Interpersonal Support Evaluation List
    IS1_V1, IS2_V1, IS3_V1, IS4_V1, IS5_V1, IS6_V1, IS7_V1, IS8_V1, IS9_V1, 
    IS10_V1, IS11_V1, IS12_V1,
    # Calculated columns from SPSS
    IS1_V1r, IS2_V1r, IS7_V1r, IS8_V1r, IS11_V1r, IS12_V1r, ISEapp_V1, ISEbel_V1, 
    ISEtan_V1,
    ...
    # Calculated columns from SPSS
    LSN_V1_family, LSN_V1_friends, LSN_V1_total,
...
Data check: Take a look at what comes after `Time_to_complete_V1`. Over time, if new variables are added to the QDS data, this could change. If it does, we may want to rename or rearrange the new columns.

```{r eval=FALSE}
# There should only be one column returned: Time_to_complete_V1
v1_reorder |>
  select(Time_to_complete_V1:last_col())
```
...
v1_reorder <- v1_reorder |> 
  select(-SUBJECT1, -TEST_V1)
```

Data check: Data dimensions

```{r}
dim(v1_reorder) # 442 808
```
...
for (i in seq_along(names(v1_reorder))) {
  attr(v1_reorder[[i]], "source") <- "Collected at Visit 1"
}
...
All the code above works, but I want to try a couple of different things in `data_survey_01_qds_import_v1.qmd` before moving on.
...
v2_reorder <- v2 %>% 
  select(
    # Administrative information
    qds_date_visit = TODAY_V2, 
    qds_visit_type = VISIT_TYPE, 
    qds_ii         = II_V2, 
    qds_visit      = VISIT_V2,
    SUBJECT, TEST_V2, READ1_V2, READ2_V2, 
    READ3_V2, CTIME_V2,
...
### v2: Drop unneeded columns

- Drop the `TEST_V2` column. I'm not sure why it exists, but it has a missing value in every row.
- Drop the `II_V1` column. I'm not sure why there is a v1 column in the v2 data.
...
Data check: Take a look at what comes after `ETIME_V2`. Over time, if new variables are added to the QDS data, this could change. If it does, we may want to rename or rearrange the new columns.

```{r eval=FALSE}
# There should only be one column returned: ETIME_V2
v2_reorder |>
  select(ETIME_V2:last_col())
```
