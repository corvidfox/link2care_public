...
```{r}
word_cb_path <- "docs/codebooks/"

walk(
  .x = c(1:5),
  .f = function(x) {
    word_cb_full_path <- paste0(word_cb_path, "L2C_V", x, " Codebook.docx")
    word_cb_content_nm <- paste0("v", x, "_word_cb_content")
    word_cb_content <- read_docx(word_cb_full_path)
    word_cb_content <- docx_summary(word_cb_content)
    assign(word_cb_content_nm, word_cb_content, envir = .GlobalEnv)
  }
)
...
word_cb_secs_cols <- get_sec_and_col_names_from_word_cb(word_cb_content)
```

Clean up the section names. For example, convert `Sect-0.*****Admin*****` to `Admin`

```{r}
word_cb_secs_cols <- clean_word_cb_sec_names(word_cb_secs_cols)
```

Create a new column in the new_name = old_name format that we can copy and paste into `select()` later.

```{r}
word_cb_secs_cols <- create_copy_paste_col(word_cb_secs_cols)
```
...
```{r eval=FALSE}
copy_and_paste <- paste(word_cb_secs_cols$copy_and_paste, collapse = ", ")
```
...
v1 <- v1 %>% 
  select(
    ...
    # Realm and  
    REALM, REALMA, REALMB, REALMC, REALMD, REALME, REALMF, REALMG, REALMH, REALMI, realm_total = REALM_S,  
   
    # Biological/Anthropometric Measures
    SQ_23, SUBJECT, WEIGHT, HEIGHT, WAIST_C, CO_V1, READ1_V1, READ2_V1, read_3 = READ3_V5, CTIME_V1,
    ...
    AF1_V1, AF2M_V1, AF3TU_V1, AF4W_V1, AF5TH_V1, AF6F_V1, AF6SA_V1, AF8SU_V1, DRINKS, AF9_V1, AF9A_V1, AF9B_V1, AF9C_V1,
...
```{r eval=FALSE}
rm(word_cb_content, word_cb_secs_cols, copy_and_paste)
```
