---
title: "Step 2 - Create Variable Map"
...
This file is step 2 in processing the source data from Link2Care (L2C) into a single data set for analysis.

This file is used to generate the organizational map linking Link2Care follow-up visit source data variables to the final combined data set.
Identification of desired variables, linkage of source data variables that capture identical data, and organization of variables into a logical order are achieved.
SPSS calculated variables were dropped in favor of recreation in R, using the processed, combined data set.

Variables from QDS, REDCap, Master Log, and TLFB were mapped.
Variables from the pre-processed DDT, Arrest log, and Bridge Session data sets were mapped.

Further data cleaning and wrangling occur in later files.
The product of this file is the initial Variable Map.
...
1.  This file creates a **variable map**.
    ...
    Specifically, variables from the QDS, REDCap, Timeline Follow Back, and Delay Discounting Task were imported and mapped.
    Limited variables from the Master Log (subject ID, group assignment, and age) were also imported for mapping.
...
2.  The columns in the data are not arranged in the order that they exist in the actual surveys for some reason.
...
```{r}
qds_spss_paths <- paste0(here("data", "qds"), "/Visit_", 1:5, "_Data.SAV")
```
...
# Data check:
# 2023-10-23: v1 imported with 442 rows and 810 columns.
# 2023-10-23: v2 imported with 406 rows and 213 columns.
# 2023-10-23: v3 imported with 273 rows and 542 columns.
# 2023-10-23: v4 imported with 207 rows and 598 columns.
# 2023-10-23: v5 imported with 178 rows and 649 columns.
```

### Data Check: Changes

We checked for changes to the source data set since this file was last modified.
To do this, we had to create a list of expected column names in each data set.
...
# 2023-10-23: REDCap imported with 119 rows and 845 columns.
...
data_mod_check(
  ...
  'orig_path_str' = here("data", "redcap", "All_Visits_Redcap.sav"),
...
The Master Log was our only source of several demographic variables, such as subject name, subject date of birth, subject age, and subject care manager.
It was also the only source of variables relating to subject continuation vs drop status, or if the research team determined the subject should be omitted from analyses.
...
# 2023-10-23: Master log imported with 442 rows and 13 columns.
...
data_mod_check(
  ...
  'num_cols' = 13,
  'col_names' = c(
      'id', 'group', 'v2_status', 'dropped_status', 'name_first', 
      'name_middle', 'name_last', 'gender', 'race', 'hispanic_latino', 
      'dob', 'age', 'care_manager'
      )
  )
# TRUE
```
...
# 2023-10-23: TLFB imported with 1768 rows and 21 columns.
...
data_mod_check(
  ...
  'orig_path_str' = here("data", "tlfb", "TLFB_Database.sav"),
...
This data was in wide format, rather than a Subject-Visit key compatible format.
Modification to achieve this format was achieved in the Step 1 file: data_survey_01_preprocess.qmd

```{r}
ddt_rds_path <- here("data", "ddt", "l2c_ddt_database_2_long.rds")
```
...
# 2023-10-23: Pre-processed DDT imported with 1768 rows and 4 columns.
# Pre-processed DDT data last modified on OneDrive 2023-10-23 09:51:22 
```
...
data_mod_check(
  ...
  'mod_dt' = '2023-10-23 09:51:22 CDT',
  'num_rows' = 1768,
  'num_cols' = 4,
  'col_names' = c(
...
Arrest Data was original in EXCEL format.
This data was in a wide Subject format, rather than Subject-Visit key format, where variables were lists of dates.

Modification into a Subject-Visit key format was achieved in the Step 1 file: data_survey_01_preprocess.qmd

```{r}
arrest_rds_path <- here("data", "arrest", "arrest_1_long.rds")
```
...
```{r message=FALSE}
arrest_data <- readRDS(arrest_rds_path)
...
# 2023-10-23: Pre-processed Arrest data imported with 985 rows and 6 columns.
# Pre-processed Arrest data last modified on OneDrive 2023-10-23 09:51:26 
```
...
data_mod_check(
  ...
  'mod_dt' = '2023-10-23 09:51:26 CDT',
  'num_rows' = 985,
  'num_cols' = 6,
  'col_names' = c(
...
Bridge Session Data was original in EXCEL format.
This data was in a wide Subject format, rather than Subject-Visit key format, where variables were lists of dates.

Modification into a Subject-Visit key format was achieved in the Step 1 file: data_survey_01_preprocess.qmd

```{r}
bridge_rds_path <- here("data", "bridge_session_data", "bridge_1_long.rds")
```
...
```{r message=FALSE}
bridge_data <- readRDS(bridge_rds_path)
...
# 2023-10-23: Pre-processed Bridge data imported with 517 rows and 26 columns.
# Pre-processed Bridge data last modified on OneDrive 2023-10-23 09:51:36
```
...
data_mod_check(
  ...
  'orig_path_str' = here("data", "bridge_session_data", "bridge_1_long.rds"),
  'mod_dt' = '2023-10-23 09:51:36 CDT',
  'num_rows' = 517,
  'num_cols' = 26,
  'col_names' = c(
      'subject', 'visit', 'reg_sessions', 'crisis_sessions', 
      'other_sessions', 'reg_minutes', 'crisis_minutes', 'other_minutes', 
      'reg_avg_duration', 'crisis_avg_duration', 'other_avg_duration', 
      'total_sessions', 'total_minutes', 'total_avg_duration', 
      'post_visits_reg_sessions', 'post_visits_crisis_sessions', 
      'post_visits_other_sessions', 'post_visits_total_sessions', 
      'post_visits_reg_minutes', 'post_visits_crisis_minutes', 
      'post_visits_other_minutes', 'post_visits_total_minutes', 
      'post_visits_reg_avg_duration', 'post_visits_crisis_avg_duration', 
      'post_visits_other_avg_duration', 'post_visits_total_avg_duration'
      )
  )
# TRUE
```
...
-   Removes the "v1", "v2", "v3", "v4", or "v5" suffix from column names regardless of if the "v" is uppercase or lowercase, or if there is or is not a preceding underscore ("\_")
...
There were 8 variables in of the Master Log variables that were appropriate for this section.

```{r}
master_log_vars <- master_log %>%
  select(all_of(c(
    "id", "group", "v2_status", "dropped_status", 
    "name_first", "name_middle", "name_last", "care_manager"
    ))
  ) %>%
  names()
...
# [1] "id"             "group"          "v2_status"      "dropped_status"
# [5] "name_first"     "name_middle"    "name_last"      "care_manager"
```
...
We identified the Delayed Discount Task (DDT) variables appropriate for this section.
...
We identified the Arrest Data variables appropriate for this section.

```{r}
arrest_vars <- arrest_data %>%
    select(all_of(c(
    'id', 'visit'
    ))
  ) %>%
...
We identified the Bridge Session variables appropriate for this section.

```{r}
bridge_vars <- bridge_data %>%
    select(all_of(c(
    'subject', 'visit'
    ))
  ) %>%
...
We set our initial desired order and standardization using the QDS variables, including a point-fix of a desired variable standardization.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "subject" in both REDCap and QDS corresponded to the item "Subject ID").
...
We identified variables to exclude: - The empty 'TEST' variables - The empty `SUBJECT1` variable - We manually checked TEST_V4 in the QDS Visit 4 data set, and determined it should be dropped as well.
...
We identified variables we wished to add to our desired variables: - The `group` variable was added from QDS Visit 3 onward - The variables introduced in the REDCap data set - The 6 variables from the Master Log not already represented - The variables from the Timeline Follow Back data
...
```{r}
# QDS "group" variable, inserted after "subject"
desired_vars <- append(
  desired_vars, 'group', after=which(desired_vars == 'subject')
  )

# Master Log demographic variables, inserted after "ii"
desired_vars <- append(desired_vars, 
                       c(
                         'name_first', 'name_middle', 'name_last',
                         'care_manager'
                         ), after=which(desired_vars == 'ii')
                       )

# REDCAP variables, with "redcap_event_name" inserted after "group"
desired_vars <- append(
  desired_vars, 'redcap_event_name', after=which(desired_vars == 'group')
  )
...
# Master Log 'status' variables
desired_vars <- append(desired_vars, c('v2_status', 'dropped_status'))

desired_vars

#  [1] "subject"            "group"              "redcap_event_name" 
#  [4] "visit"              "visit_type"         "today"             
#  [7] "ii"                 "name_first"         "name_middle"       
# [10] "name_last"          "care_manager"       "ctime"             
# [13] "endtime"            "etime"              "maca_mdd_timestamp"
# [16] "maca_mdd_complete"  "ps_timestamp"       "ps_complete"       
# [19] "email"              "days_followed"      "flag_outofwindow"  
# [22] "v2_status"          "dropped_status"
```
...
The limited variables from the DDT, Arrest, and Bridge Session data sets were added manually for brevity.
  ...
  full_join(tibble::tibble(
    'master_log' = (
      master_log %>%
        select(all_of(c(
          'id', 'group', 'v2_status', 'dropped_status', 'name_first',
          'name_middle', 'name_last', 'care_manager'
          ))
          ) %>%
        names()
    ),
    'variable' = c(
        'subject', 'group', 'v2_status', 'dropped_status', 'name_first',
        'name_middle', 'name_last', 'care_manager'
        )
    ),
  by = "variable"
...
We added the section to our overall list.
We purged the instrument and section maps to reduce memory burden in processing and protect against potential memory leak.
...
Screening was performed with 5 instruments: Screening Questionnaire, Mini-Mental State Exam, Rapid Estimate of Adult Literacy in Medicine (REALM), Biological Measures/Anthropometrics, and a Locator Form.
The Locator Form was excluded from analysis.
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that 26 of our 44 variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "sq_2" in both REDCap and QDS corresponded to the item "Are you Hispanic or Latino?").

We determined that QDS item `sq_7` corresponded to REDCap item `sq_7b2` and `sq_7b1` was an addition in the REDCap data, as the item was divided in the REDCap format.
The dummy categorical variables for the select all that apply questions `sq_18` and `sq_22` would require manual renaming, as they were not compatible with our standardization function.
...
We dropped the `gender` variable which only contained missing values, in favor of standardizing the `visit_gender` variable which did contain the pertinent values to `gender`.
We added the new `sq_7b_1` variable to our list of desired variables after our QDS `sq_7b`, and planned to map the REDCap `sq_7b_2` to the QDS `sq_7b`.
...
We identified the 5 variables in the Master Log which corresponded to items present in the QDS data: age, race, ethnicity, and gender.

```{r}
master_log_vars <- master_log %>%
  select(all_of(c(
    "gender", "race", "hispanic_latino", "age", "dob"
    ))
  ) %>%
  names()
...
# [1] "gender"          "race"            "hispanic_latino" "age"            
# [5] "dob"    
```

We created the required point-fixes to insert the desired version of these Master Log variables in the appropriate order within our desired variables.

```{r}
# Master Log 'gender' is equivalent to 'gender' in screening
desired_vars <- append(
  desired_vars, 'ml_gender', after=which(desired_vars == 'gender')
  )

# Master Log 'hispanic_latino' is equivalent to 'sq_2' in screening
desired_vars <- append(
  desired_vars, 'ml_hispanic_latino', after=which(desired_vars == 'sq_2')
  )

# Master Log 'race' is equivalent to 'sq_3' in screening
desired_vars <- append(
  desired_vars, 'ml_race', after=which(desired_vars == 'sq_3')
  )

# Master Log 'age', calculated from 'dob', is equivalent to 'sq_4' in screening
desired_vars <- append(
  desired_vars, c('ml_dob', 'ml_age'), after=which(desired_vars == 'sq_4')
  )

desired_vars

#  [1] "gender"             "ml_gender"          "sq_2"              
#  [4] "ml_hispanic_latino" "sq_3"               "ml_race"           
#  [7] "sq_4"               "ml_dob"             "ml_age"            
# [10] "sq_5"               "sq_6"               "sq_7"              
# [13] "sq_7a"              "sq_7b"              "sq_7b_1"           
# [16] "sq_7c"              "sq_7d"              "sq_8"              
# [19] "sq_9"               "sq_9l"              "sq_10"             
# [22] "sq_11"              "sq_12"              "sq_13"             
# [25] "sq_14"              "sq_15"              "sq_16"             
# [28] "sq_17"              "sq_18"              "sq_18a"            
# [31] "sq_18b"             "sq_18c"             "sq_18d"            
# [34] "sq_18e"             "sq_18f"             "sq_18g"            
# [37] "sq_18h"             "sq_18i"             "sq_19"             
# [40] "sq_20"              "sq_21"              "sq_22"             
# [43] "sq_22a"             "sq_22b"             "sq_22c"            
# [46] "sq_22d"             "sq_22e"             "sq_22f"            
# [49] "sq_22g"             "sq_22h"             "sq_23" 
```

We created our source maps, including the required point-fixes.
Master Log variables were manually standardized due to significant differences in source set naming.
...
source_maps <- vm_make_source_map(
  ...
  full_join(tibble::tibble(
    ...
    'variable' = c(
        'ml_gender', 'ml_race',  'ml_hispanic_latino', 'ml_age', 'ml_dob'
        )
    ),
  by = "variable"
  ) %>%
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that 14 of our 41 variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "mms_1a" in both REDCap and QDS corresponded to the item "What year is it?").

We identified `mms_score`, `MMS_total`, and `MMS_severity` as SPSS calculated variables we wished to drop.
We also found variable name variations in both QDS Visit 5 and the REDCap data which evaded our standardization function, and would require manual assignment.
...
source_maps <- vm_make_source_map(
  ...
  mutate(variable = case_when(
    ...
    variable == 'MMS4_V5' ~ 'mms_4v',
    variable == 'MMS_4_V5' ~ 'mms_4',
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that the variable `realm_score` only contained missing values.
...
We set our initial desired order and standardization based on the QDS variables.
We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "realm_a" in QDS and "ream_0" in REDCap corresponded to the item "Fat").
We found that our REDCap variables all escaped our standardization function.
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that all three of our variables only contained missing values.
...
We set our initial desired order and standardization utilizing the QDS variables.
We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "read_1" in both REDCap and QDS corresponded to the item "Do you use reading glasses?").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that 12 of our 56 variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "dem_1" in both REDCap and QDS corresponded to the item "What is your present marital status?").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that `bh18b1v1` only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "bh_1" in both REDCap and QDS corresponded to the item "What is the total amount of time you have been homeless in your lifetime? (for example 3 and a half years would be 3 years and 6 months)").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "sss_1" in both REDCap and QDS corresponded to the item "Community Ladder").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "hs_1" in both REDCap and QDS corresponded to the item "In general, would you say your health is:").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that `s36_v1` only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "ds_1" in both REDCap and QDS corresponded to the item "During the last 3 months, did you use larger amounts of drugs and/or alcohol or use them for a longer time than you planned or intended?").
...
We checked to see if any of these variables were only missing values in the REDCap.
All four of our variables were missing values.
...
We checked to see if any of these variables were only missing values in the REDCap.
All of the variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "brac_1" in both REDCap and QDS corresponded to the item "Now, looking back before you were 18 years of age did you live with anyone who was depressed, mentally ill, or suicidal?").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that 13 of our 97 variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "t_1" in both REDCap and QDS corresponded to the item "Have you smoked at least 100 cigarettes (or cigarillos) in your entire life?
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "hsi_1" in both REDCap and QDS corresponded to the item "How many cigarettes per day do you smoke?").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "brs_1" in both REDCap and QDS corresponded to the item "During the past 30 days, for how many days have you felt you did not get enough rest or sleep?").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that `af9c_v1` contained only missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "af_1" in both REDCap and QDS corresponded to the item "Have you consumed alcohol (including beer and wine) in the past 3 years?"
).
...
We created our source map.
As this only composed a single source (thus named variables and desired variables would be in the same order), we manually created the tibble for brevity.
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that all 7 of our variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "pbq_1" in both REDCap and QDS corresponded to the item "I should do whatever I can get away with.").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "fss_1" in both REDCap and QDS corresponded to the item "In the last month, the food that you bought just didn't last, and you didn't have money to get more.").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "cj_1" in both REDCap and QDS corresponded to the item "Have you used drugs or alcohol in the past 12 months?").
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that 8 of our 15 variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "sb_1" in both REDCap and QDS corresponded to the item "Have you ever had any kind of sex, including vaginal, anal, or oral?").
We identified the REDCap variables that contained a different naming convention.
...
We identified our keys and identified the matching variables in the QDS Visit 2 set.
MacAurthur Major Discrimination was not used in QDS Visit 1.
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that all 14 of our variables only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "mmd_1a" in both REDCap and QDS corresponded to the item "You were discouraged by a teacher or advisor from seeking higher education?").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "uls_1" in both REDCap and QDS corresponded to the item "How much stress do you generally experience related to money or finances?").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "pv_1" in both REDCap and QDS corresponded to the item "In the past 30 days, has anyone used violence, such as in a mugging, fight, or sexual assault, against you?").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "ps_1" in both REDCap and QDS corresponded to the item "In the last week, how often have you felt that you were unable to control the important things in your life?").
We only found the previously identified SPSS calculated variables.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "dts_1" in both REDCap and QDS corresponded to the item "Feeling distressed or upset is unbearable to me.").
We only found the previously identified SPSS calculated variables.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "aq_1" in both REDCap and QDS corresponded to the item "Given enough provocation, I may hit another person.").
We only found the previously identified SPSS calculated variables.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "ces_1" in both REDCap and QDS corresponded to the item "During the past week, I was bothered by things that don't usually bother me.").
We only found the previously identified SPSS calculated variables.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "rp_1" in both REDCap and QDS corresponded to the item "Apart from special occasions such as weddings and funerals, how often do you attend religious services?").
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "lsn_1" in both REDCap and QDS corresponded to the item "Considering the people to who you are related by birth, marriage, adoption etc, how many relatives do you see or hear from at least once a month?").
We only found the previously identified SPSS calculated variables.
...
We added the instrument to our section-wise list of instruments.
We purged the instrument map to reduce memory burden in processing and protect against potential memory leak.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "r_1" in both REDCap and QDS corresponded to the item "Over the past 30 days, how many days did you receive Mental Health/Behavioral Health Counseling at local shelters?").
We identified the REDCap variables with a different naming convention.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "bpm_1" in both REDCap and QDS corresponded to the item "I am comfortable with completing case management sessions in my case manager's office at the shelter.").
We identified the REDCap variables that used a different naming convention.
...
The section for Treatment Quality and Satisfaction contained two instruments: the Treatment Quality and Satsifaction survey, and the Delayed Discount Task (DDT).
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "tq_1_1" in both REDCap and QDS corresponded to the item "How helpful has the Bridge Homeless Recovery Program been to you?").
We identified that the REDCap data did not have the same spelling error ('yq' instead of 'tq') present in the QDS data.
...
```{r}
sec_instruments <- append(sec_instruments, list('tq' = inst_map))
...
We identified the variables present in our pre-processed DDT data set.

```{r}
colnames(ddt)

# [1] "Subject" "visit"   "K"       "ED50"   
```
...
for (var in colnames(ddt)){
  if(sum(!is.na(ddt[,var])) == 0){
    missing_vars <- c(missing_vars, var)
    message <- c(message, (paste(var, "is only missing values")))
  }
}
...
We set our initial desired order and standardization.
We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
Due to the format of these variables, our standardization function would fail.
As such, we performed manual standardization.
...
```{r}
inst_map <- vm_create_for_instrument(
  'Delay Discount Task (DDT)', desired_vars
  ) %>%
  left_join(source_maps, 
            by = 'variable')
...
```{r}
sec_instruments <- append(sec_instruments, list('ddt' = inst_map))
...
sec_map <- vm_join_inst_section(
  'Treatment Quality and Satisfaction', sec_instruments
  )

sec_map
```

We added the section to our overall list, and purged the section map to reduce memory burden in processing.
...
Staff Perceptions of the Link2Care was not included in the QDS or REDCap data.

## Ecological Momentary Assessment (EMA) Matrix 🔴 - PLACEHOLDER

The Ecological Momentary Assessment variables were not included in the QDS or REDCap data.

## COVID-19 Pandemic Related Data

The section of COVID-19 Pandemic Related Data contained one instrument: COVID-19 Vaccination Questionnaire.
...
We checked to see if any of these variables were only missing values in the REDCap.
We found that `vac2` only contained missing values.
...
We checked for any variables that would either standardize differently, or were not already identified.
We manually verified that similar variables corresponded to the same question (i.e. "vac_1" in both REDCap and QDS corresponded to the item "Have you received the COVID-19 vaccine?").
...
The Arrest Data was separate from the other sections, and was considered a single instrument section.

We identified the variables present in our pre-processed Arrest data set.

```{r}
colnames(arrest_data)

# [1] "id"                            "arrested_during_followup"     
# [3] "arrest_count_after_visits"     "arrest_count_total"           
# [5] "visit"                         "arrest_count_since_prev_visit"
```
...
for (var in colnames(arrest_data)){
  if(sum(!is.na(arrest_data[,var])) == 0){
    missing_vars <- c(missing_vars, var)
    message <- c(message, (paste(var, "is only missing values")))
  }
}
...
We set our initial desired order and standardization.
We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
Since our variables already had the 'arrest' prefix, we could use these variables directly.

```{r}
arrest_vars <- arrest_data %>%
  select(all_of(c(
    'arrested_during_followup', 'arrest_count_since_prev_visit',
    'arrest_count_after_visits', 'arrest_count_total'
    ))
  ) %>%
  names()

desired_vars <- arrest_vars

desired_vars

# [1] "arrested_during_followup"      "arrest_count_since_prev_visit"
# [3] "arrest_count_after_visits"     "arrest_count_total" 
```
...
As the Arrest Data section was a single instrument, we created our section.

```{r}
sec_map <- vm_join_inst_section('Arrest Data', list(inst_map))
...
```{r}
all_sections <- append(all_sections, list('arrest' = sec_map))
rm(inst_map)
rm(sec_map)
```
...
The Bridge Session Data was separate from the other sections, and was considered a single instrument section.

We identified the variables present in our pre-processed Bridge Session data set.

```{r}
colnames(bridge_data)

#  [1] "subject"                         "visit"                          
#  [3] "reg_sessions"                    "crisis_sessions"                
#  [5] "other_sessions"                  "reg_minutes"                    
#  [7] "crisis_minutes"                  "other_minutes"                  
#  [9] "reg_avg_duration"                "crisis_avg_duration"            
# [11] "other_avg_duration"              "total_sessions"                 
# [13] "total_minutes"                   "total_avg_duration"             
# [15] "post_visits_reg_sessions"        "post_visits_crisis_sessions"    
# [17] "post_visits_other_sessions"      "post_visits_total_sessions"     
# [19] "post_visits_reg_minutes"         "post_visits_crisis_minutes"     
# [21] "post_visits_other_minutes"       "post_visits_total_minutes"      
# [23] "post_visits_reg_avg_duration"    "post_visits_crisis_avg_duration"
# [25] "post_visits_other_avg_duration"  "post_visits_total_avg_duration" 
```
...
for (var in colnames(bridge_data)){
  if(sum(!is.na(bridge_data[,var])) == 0){
    missing_vars <- c(missing_vars, var)
    message <- c(message, (paste(var, "is only missing values")))
  }
}
...
We set our initial desired order and standardization.
We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
We wished to have "bridge" appended to these variables as a prefix

```{r}
bridge_vars <- bridge_data %>%
  select(all_of(c(
    'reg_sessions', 'crisis_sessions', 
    'other_sessions', 'total_sessions', 'reg_minutes', 
    'crisis_minutes', 'total_minutes',
    'other_minutes', 'reg_avg_duration', 'crisis_avg_duration', 
    'other_avg_duration',  'total_avg_duration', 'post_visits_reg_sessions', 
...
desired_vars <- paste0("bridge_", bridge_vars)

desired_vars

#  [1] "bridge_reg_sessions"                   
#  [2] "bridge_crisis_sessions"                
#  [3] "bridge_other_sessions"                 
#  [4] "bridge_total_sessions"                 
#  [5] "bridge_reg_minutes"                    
#  [6] "bridge_crisis_minutes"                 
#  [7] "bridge_total_minutes"                  
#  [8] "bridge_other_minutes"                  
#  [9] "bridge_reg_avg_duration"               
# [10] "bridge_crisis_avg_duration"            
# [11] "bridge_other_avg_duration"             
# [12] "bridge_total_avg_duration"             
# [13] "bridge_post_visits_reg_sessions"       
# [14] "bridge_post_visits_crisis_sessions"    
# [15] "bridge_post_visits_other_sessions"     
# [16] "bridge_post_visits_total_sessions"     
# [17] "bridge_post_visits_reg_minutes"        
# [18] "bridge_post_visits_crisis_minutes"     
# [19] "bridge_post_visits_other_minutes"      
# [20] "bridge_post_visits_total_minutes"      
# [21] "bridge_post_visits_reg_avg_duration"   
# [22] "bridge_post_visits_crisis_avg_duration"
# [23] "bridge_post_visits_other_avg_duration" 
# [24] "bridge_post_visits_total_avg_duration"
```
...
```{r}
inst_map <- vm_create_for_instrument(
  'Bridge Session Data', desired_vars
  ) %>%
  left_join(source_maps, 
            by = 'variable')
...
As the Bridge Session Data section was a single instrument, we created our section.

```{r}
sec_map <- vm_join_inst_section('Bridge Session Data', list(inst_map))
...
```{r}
all_sections <- append(all_sections, list('bridge' = sec_map))
rm(inst_map)
rm(sec_map)
```
...
We purged containers that were no longer necessary to reduce memory burden and protect in case of memory leak.
...
We examined the 65 variables that had otherwise failed to be assigned from each data set.
These variables were all determined to be either SPSS calculated variables, or entirely empty errant duplicates of variables, that we wished to recreate in R from our aggregated set.
...
#  [1] "TEST_V1"                 "SUBJECT1"               
#  [3] "BRIS2_QD_dichot"         "QD_unintent_slp"        
#  [5] "PA"                      "VA"                     
#  [7] "A"                       "HA"                     
#  [9] "AlcQF_V1"                "binge_drinker_V1"       
# [11] "Heavy_drinker"           "current_months_homeless"
# [13] "friends_smoke_V1"        "spouse_smoke"           
# [15] "Mental_Health_dx"        "pain"                   
# [17] "Substance_abuse_hs"      "Substance_use_30d"      
# [19] "Any_health_insurance"    "Time_to_complete_V1"    
# [21] "TCU_DS_TOTAL_V2"         "TCU_DS_SCORE_V2"        
# [23] "TCU_desire_v2_total"     "TCU_tn_v2_total"        
# [25] "TCU_ts_v2_total"         "TCU_se_v2_total"        
# [27] "TCU_hs_v2_total"         "TCU_rt_v2_total"        
# [29] "Time_to_complete_V2"     "BRIS2_V3_dichot"        
# [31] "unintent_slp_V3"         "AlcQF_V3"               
# [33] "binge_drinker_V3"        "Heavy_drinker_V3"       
# [35] "PA_V3"                   "VA_V3"                  
# [37] "A_V3"                    "HA_V3"                  
# [39] "Time_to_complete_V3"     "PHQ_dep_dichot_total_V4"
# [41] "BRIS2_V4_dichot"         "unintent_slp_V4"        
# [43] "AlcQF_V4"                "binge_drinker_V4"       
# [45] "Heavy_drinker_V4"        "TCU_desire_v4_total"    
# [47] "TCU_tn_v4_total"         "TCU_ts_V4_total"        
# [49] "PA_V4"                   "VA_V4"                  
# [51] "A_V4"                    "HA_V4"                  
# [53] "Time_to_complete_V4"     "PHQ_dep_dichot_total_V5"
# [55] "BRIS2_V5_dichot"         "unintent_slp_V5"        
# [57] "AlcQF_V5"                "binge_drinker_V5"       
# [59] "Heavy_drinker_V5"        "PA_V5"                  
# [61] "VA_V5"                   "A_V5"                   
# [63] "HA_V5"                   "Time_to_complete_V5"    
# [65] "staffperc_q1"
```
...
We purged the remaining containers that were no longer necessary to reduce memory burden and protect against memory leak.
...
# Capturing Variable Attributes

We wished to capture the label and labels attributes from our source data sets, to ensure they are uniform throughout processing.

We renamed our data sets to facilitate easier use of dplyr and tidyverse functions.
We added a "qds" prefix to the QDS visit columns in our variable map.

```{r}
# Rename QDS cols in variable map to have a "qds_" prefix
variable_map <- variable_map %>%
  rename_at(
    c('v1', 'v2', 'v3', 'v4', 'v5'), 
    ~c('qds_v1', 'qds_v2', 'qds_v3', 'qds_v4', 'qds_v5')
    )

#  Rename source data frames
redcap_df <- redcap
tlfb_df <- tlfb
ddt_df <- ddt

# Remove old data frame names from global
rm(redcap)
rm(tlfb)
rm(ddt)
```
...
We extracted and compared the labels for each variable in our source data sets.
The 16 variables with more than one label across the data sets were isolated for manual review.

```{r}
attribute_list <- variable_map %>%
  select(variable) %>%
  rowwise() %>%
  mutate(
    v1_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$qds_v1),
      ifelse(
        !is.null(attributes(v1[[variable_map[variable_map$variable ==
                                               variable,]$qds_v1]])$label),
        attributes(v1[[variable_map[variable_map$variable ==
                                               variable,]$qds_v1]])$label,
        NA),
      NA
      ),
    v2_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$qds_v2),
      ifelse(
        !is.null(attributes(v2[[variable_map[variable_map$variable ==
                                               variable,]$qds_v2]])$label),
        attributes(v2[[variable_map[variable_map$variable ==
                                               variable,]$qds_v2]])$label,
        NA),
      NA
      ),
    v3_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$qds_v3),
      ifelse(
        !is.null(attributes(v3[[variable_map[variable_map$variable ==
                                               variable,]$qds_v3]])$label),
        attributes(v3[[variable_map[variable_map$variable ==
                                               variable,]$qds_v3]])$label,
        NA),
      NA
      ),
    v4_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$qds_v4),
      ifelse(
        !is.null(attributes(v4[[variable_map[variable_map$variable ==
                                               variable,]$qds_v4]])$label),
        attributes(v4[[variable_map[variable_map$variable ==
                                               variable,]$qds_v4]])$label,
        NA),
      NA
      ),
    v5_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$qds_v5),
      ifelse(
        !is.null(attributes(v5[[variable_map[variable_map$variable ==
                                               variable,]$qds_v5]])$label),
        attributes(v5[[variable_map[variable_map$variable ==
                                               variable,]$qds_v5]])$label,
        NA),
      NA
      ),
    redcap_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$redcap),
      ifelse(
        !is.null(attributes(redcap_df[[variable_map[variable_map$variable ==
                                               variable,]$redcap]])$label),
        attributes(redcap_df[[variable_map[variable_map$variable ==
                                               variable,]$redcap]])$label,
        NA),
      NA
      ),
    tlfb_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$tlfb),
      ifelse(
        !is.null(attributes(tlfb_df[[variable_map[variable_map$variable ==
                                               variable,]$tlfb]])$label),
        attributes(tlfb_df[[variable_map[variable_map$variable ==
                                               variable,]$tlfb]])$label,
        NA),
      NA
      ),
    ddt_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$ddt),
      ifelse(
        !is.null(attributes(ddt_df[[variable_map[variable_map$variable ==
                                               variable,]$ddt]])$label),
        attributes(ddt_df[[variable_map[variable_map$variable ==
                                               variable,]$ddt]])$label,
        NA),
      NA
      ),
    arrest_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$arrest),
      ifelse(
        !is.null(attributes(arrest_data[[variable_map[variable_map$variable ==
                                               variable,]$arrest]])$label),
        attributes(arrest_data[[variable_map[variable_map$variable ==
                                               variable,]$arrest]])$label,
        NA),
      NA
      ),
    bridge_label = ifelse(
      !is.na(variable_map[variable_map$variable == variable,]$bridge),
      ifelse(
        !is.null(attributes(bridge_data[[variable_map[variable_map$variable ==
                                               variable,]$bridge]])$label),
        attributes(bridge_data[[variable_map[variable_map$variable ==
                                               variable,]$bridge]])$label,
        NA),
      NA
      )
  ...
  mutate(
    count = n_distinct(c_across(ends_with("label")), na.rm=TRUE)
  ) %>%
  ungroup()

checking <- attribute_list %>%
  filter(count > 1)
```

We determined that there were 3 variables (`bh_14`, `t_10`, and `dd_10`) with significantly different labels.
The remaining variables had slight punctuation differences which did not change the meaning of the label.

We chose the QDS V1 labels to form our initial labels.
Other data sets would be utilized if QDS V1 was empty.

```{r}
attribute_labels <- attribute_list %>%
  # Point fixes of 3 variables with variants in labels
  mutate(v1_label = case_when(
    variable == 'bh_14' ~ paste(
      "Were you arrested or booked for breaking the law in the PAST",
       "[Interval since last visit, 1 month for baseline]?"
      ),
    variable == 't_10' ~ paste(
      "In the past 30 days [1 year for baseline],",
      "how many times have you successfully quit smoking for at least 24", 
      "hours (only include times where you wanted to quit smoking– do not",
      "include times when you wanted to smoke but didn’t have money to",
      "buy cigarettes)?"
      ),
    variable == 'dd_10' ~ 
      "What was the main reason for the discrimination you experienced?",
    TRUE ~ v1_label
    )
  ) %>%
  # Fill blanks with existing values, in preference order
  mutate(v1_label = ifelse(is.na(v1_label), 
                            case_when(
                              !is.na(v2_label) ~ v2_label,
                              !is.na(v3_label) ~ v3_label,
                              !is.na(v4_label) ~ v4_label,
                              !is.na(v5_label) ~ v5_label,
                              !is.na(redcap_label) ~ redcap_label,
                              !is.na(tlfb_label) ~ tlfb_label,
                              !is.na(ddt_label) ~ ddt_label,
                              !is.na(arrest_label) ~ arrest_label,
                              !is.na(bridge_label) ~ bridge_label,
                              TRUE ~ v1_label
                              ), 
                            v1_label
                           )
         ) %>%
  # Rename column to facilitate updating values
  rename_at('v1_label', ~'attr_label') %>%
  # Isolate variable key and attribute label column
  select(variable, attr_label)
```

There were 40 variables that continued to lack a label.
We isolated these variables for additional review.

```{r}
checking <- pull(
   attribute_labels %>%
    filter(is.na(attr_label)) %>%
    select(variable)
)
checking  
#  [1] "redcap_event_name"       "today"                  
#  [3] "name_first"              "name_middle"            
#  [5] "name_last"               "care_manager"           
#  [7] "ctime"                   "etime"                  
#  [9] "maca_mdd_timestamp"      "maca_mdd_complete"      
# [11] "ps_timestamp"            "ps_complete"            
# [13] "email"                   "days_followed"          
# [15] "flag_outofwindow"        "v2_status"              
# [17] "dropped_status"          "ml_gender"              
# [19] "ml_hispanic_latino"      "ml_race"                
# [21] "ml_dob"                  "ml_age"                 
# [23] "sq_7b_1"                 "totaldrinks"            
# [25] "num_alc_abstinent_days"  "num_drinking_days"      
# [27] "ave_drinks_day"          "heavy_drinking_days"    
# [29] "greatest_drinks_1_day"   "drinks_wk"              
# [31] "num_days_using_drugs"    "per_days_using_drugs"   
# [33] "per_drug_abstinent_days" "drug_use_days_yr"       
# [35] "drug_use_days_week"      "num_days_homeless"      
# [37] "per_days_homeless"       "num_days_home"          
# [39] "est_days_homeless_yr"    "avg_days_homeless_wk"
```

We manually applied labels to these variables.

```{r}
logic_gate <- identical(
  sort(checking),
  sort(c(
    'redcap_event_name', 'today', 'name_first', 'name_middle', 'name_last',
    'care_manager', 'ctime', 'etime', 'maca_mdd_timestamp',
    'maca_mdd_complete', 'ps_timestamp', 'ps_complete', 'email',
    'days_followed', 'flag_outofwindow', 'v2_status', 'dropped_status',
    'ml_gender', 'ml_hispanic_latino', 'ml_race', 'ml_dob', 'ml_age',
    'sq_7b_1', 'totaldrinks', 'num_alc_abstinent_days', 'num_drinking_days',
    'ave_drinks_day', 'heavy_drinking_days', 'greatest_drinks_1_day',
    'drinks_wk', 'num_days_using_drugs', 'per_days_using_drugs',
    'per_drug_abstinent_days', 'drug_use_days_yr', 'drug_use_days_week',
    'num_days_homeless', 'per_days_homeless', 'num_days_home',
...
if(!logic_gate){
  stop("Variables have changed - check before continuing!")
}
if(logic_gate){
  attribute_labels <-attribute_labels %>%
    mutate(attr_label = case_when(
      variable == 'redcap_event_name' ~ 'Collection event name in REDCap',
      variable == 'today' ~ 'Date of data collection event',
      variable == 'name_first' ~ 'Subject name (first)',
      variable == 'name_middle' ~ 'Subject name (middle initial)', 
      variable == 'name_last' ~ 'Subject name (last)',
      variable == 'care_manager' ~"Subject's Care manager", 
      variable == 'ctime' ~ 'Start time of Data collection',
      variable == 'etime' ~ 'Elapsed time in data collection',
      variable == 'maca_mdd_timestamp' ~ paste(
        'Start time for MacAurthur Major Descrimination instrument'
        ),
      variable == 'maca_mdd_complete' ~paste(
        'End time for MacAurthur Major Descrimination instrument'
        ),
      variable == 'ps_timestamp' ~ paste(
        'Start time for Perceived Stress instrument'
        ),
      variable == 'ps_complete' ~ paste(
        'End time for Perceived Stress instrument'
        ),
      variable == 'email' ~ 'Subject email address',
      variable == 'days_followed' ~ paste(
        'Number of days followed in TLFB collection period'
        ),
      variable == 'flag_outofwindow' ~ paste(
        'Flag if TLFB collection was outside of the desired date window'
        ),
      variable == 'v2_status' ~ paste(
        'Status after Visit 2 (randomized, dropped, etc.)'
        ),
      variable == 'dropped_status' ~ paste(
        'When was subject dropped from follow-up?'
        ),
      variable == 'ml_gender' ~ 'Subject gender (Master Log)',
      variable == 'ml_hispanic_latino' ~ paste(
        'Is subject hispanic or latino (Master Log)?'
        ),
      variable == 'ml_race' ~ 'Subject Race (Master Log)',
      variable == 'ml_dob' ~ 'Subject date of birth (Master Log)',
      variable == 'ml_age' ~ 'Subject age in years (Master Log)',
      variable == 'sq_7b_1' ~ paste(
        'Did you receive a study flyer from the Dallas County Jail?'
        ),
      variable == 'totaldrinks' ~ paste(
        'Total number of alcoholic drinks in TLFB collection period'
        ),
      variable == 'num_alc_abstinent_days' ~ paste(
       "Total number of days without the consumption of any alcohol",
       "in TLFB collection period" 
        ),
      variable == 'num_drinking_days' ~ paste(
       "Total number of days with any amount of alcohol consumption",
       "in TLFB collection period"
        ),
      variable == 'ave_drinks_day' ~ paste(
      "Average number of alcoholic drinks per day", 
       "in TLFB collection period"
        ),
      variable == 'heavy_drinking_days' ~ paste(
        "Total number of days where more than (4 alcoholic drinks", 
        "for females, 5 alcoholic drinks for males)",
        "were consumed in one day,",
        "in TLFB collection period"
        ),
      variable == 'greatest_drinks_1_day' ~ paste(
        "Largest number of alcoholic drinks consumed in one day", 
        "in TLFB collection period"
        ),
      variable == 'drinks_wk' ~ paste(
        "Average number of alcoholic drinks consumed per week", 
         "in TLFB collection period"
        ),
      variable == 'num_days_using_drugs' ~ paste(
        "Total number of days with any amount of drug use",
         "in TLFB collection period"
        ),
      variable == 'per_days_using_drugs' ~ paste(
        "Percentage of days with any amount of drug use", 
         "in TLFB collection period"
        ),
      variable == 'per_drug_abstinent_days' ~ paste(
        "Percentage of days without any amount of drug use", 
         "in TLFB collection period"
        ),
      variable == 'drug_use_days_yr' ~ paste(
        "Average number of days per year with any amount of drug use", 
        "based on the TLFB collection period"
        ),
      variable == 'drug_use_days_week' ~ paste(
        "Average number of days per week with any amount of drug use", 
        "based on the TLFB collection period"
        ),
      variable == 'num_days_homeless' ~ paste(
        "Total number of nights with reported homelessness", 
        "in TLFB collection period"
        ),
      variable == 'per_days_homeless' ~ paste(
        "Percentage of nights with reported homelessness", 
        "in TLFB collection period"
        ),
      variable == 'num_days_home' ~ paste(
        "Total number of nights spent at home", 
        "in TLFB collection period"
        ),
      variable == 'est_days_homeless_yr' ~ paste(
        "Average number of nights spent homless per year", 
        "based on TLFB collection period"
        ),
      variable == 'avg_days_homeless_wk' ~ paste(
        "Total number of days with reported homelessness", 
         "in TLFB collection period"
        ),
      TRUE ~ attr_label
    )
...
We confirmed there were no remaining variables without a label.
...
## Extracting Variable Value Labels

We repeated a similar process for the variable value labels.
These value labels were absent from the REDCap, Master Log, TLFB, DDT, Arrest, and Bridge Session data sets.
...
attribute_list <- variable_map %>%
  ...
  mutate(
    v1_labels = ifelse(
      ...
      ifelse(
        !is.null(attributes(v1[[variable_map[variable_map$variable ==
                                               variable,]$qds_v1]])$labels),
        list(attributes(v1[[variable_map[variable_map$variable ==
                                               variable,]$qds_v1]])$labels),
        NA),
      NA
      ),
    v2_labels = ifelse(
      ...
      ifelse(
        !is.null(attributes(v2[[variable_map[variable_map$variable ==
                                               variable,]$qds_v2]])$labels),
        list(attributes(v2[[variable_map[variable_map$variable ==
                                               variable,]$qds_v2]])$labels),
        NA),
      NA
      ),
    v3_labels = ifelse(
      ...
      ifelse(
        !is.null(attributes(v3[[variable_map[variable_map$variable ==
                                               variable,]$qds_v3]])$labels),
        list(attributes(v3[[variable_map[variable_map$variable ==
                                               variable,]$qds_v3]])$labels),
        NA),
      NA
      ),
    v4_labels = ifelse(
      ...
      ifelse(
        !is.null(attributes(v4[[variable_map[variable_map$variable ==
                                               variable,]$qds_v4]])$labels),
        list(attributes(v4[[variable_map[variable_map$variable ==
                                               variable,]$qds_v4]])$labels),
        NA),
      NA
      ),
    v5_labels = ifelse(
      ...
      ifelse(
        !is.null(attributes(v5[[variable_map[variable_map$variable ==
                                               variable,]$qds_v5]])$labels),
        list(attributes(v5[[variable_map[variable_map$variable ==
                                               variable,]$qds_v5]])$labels),
        NA),
      NA
      )
  ...
  add_column(count = 0) %>%
  # Get count of the number of unique versions of these labels
  mutate(
      # Compare QDS V1 to everything else, if QDS V1 exists
          count = ifelse((!is.null(v1_labels) & !is.null(v2_labels)),
                         ifelse(!identical(v1_labels, v2_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v1_labels) & !is.null(v3_labels)),
                         ifelse(!identical(v1_labels, v3_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v1_labels) & !is.null(v4_labels)),
                         ifelse(!identical(v1_labels, v4_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v1_labels) & !is.null(v5_labels)),
                         ifelse(!identical(v1_labels, v5_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         # Compare QDS V2 to everything else, if QDS V2 exists
         count = ifelse((!is.null(v2_labels) & !is.null(v3_labels)),
                         ifelse(!identical(v2_labels, v3_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v2_labels) & !is.null(v4_labels)),
                         ifelse(!identical(v2_labels, v4_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v2_labels) & !is.null(v5_labels)),
                         ifelse(!identical(v2_labels, v5_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
  # Compare QDS V3 to everything else, if QDS V3 exists
         count = ifelse((!is.null(v3_labels) & !is.null(v4_labels)),
                         ifelse(!identical(v3_labels, v4_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
         count = ifelse((!is.null(v3_labels) & !is.null(v5_labels)),
                         ifelse(!identical(v3_labels, v5_labels),
                            count + 1,
                            count
                            ),
                          count
                          ),
  # Compare QDS V4 to everything else, if QDS V4 exists
         count = ifelse((!is.null(v4_labels) & !is.null(v5_labels)),
                         ifelse(!identical(v4_labels, v5_labels),
                            count + 1,
                            count
                            ),
                          count
...
In manually checking the variable with more than one set of values (`read_3`), we discovered that this variation was due to different punctuation typos which did not change the meaning of the label.

We chose the QDS V1 variable value labels to form our initial variable value labels.
Other data sets would be utilized if QDS V1 was empty.
...
attribute_list <- attribute_list %>%
  # Fill blanks with existing values, in preference order
  rowwise() %>%
  mutate(v1_labels = ifelse(is.null(v1_labels), 
                             case_when(
                                !is.null(v2_labels) ~ list(v2_labels),
                                !is.null(v3_labels) ~ list(v3_labels),
                                !is.null(v4_labels) ~ list(v4_labels),
                                !is.null(v5_labels) ~ list(v5_labels),
                                TRUE ~ list(NULL)
                                ),
                            list(v1_labels)
                            )
  ...
  ungroup() %>%
  rename_at('v1_labels', ~'attr_var_labels') %>%
  select(variable, attr_var_labels)
```

We combined these into a single attribute list.

```{r}
attribute_list <- left_join(attribute_labels, attribute_list, by='variable')
```

## Merging into Variable Map

We then merged these values into our Variable Map.

```{r}
variable_map <- left_join(variable_map, attribute_list, by = 'variable')

attr(variable_map[['attr_label']], 'label') <- "Variable label"

attr(variable_map[['attr_var_labels']], 'label') <- "Value labels"
```

### Data Check

We ensured our Variable Map met our standards.
...
variable_map_path <- here(
  "data", "Combined Participant Data", "variable_map_01.xlsx"
  )
