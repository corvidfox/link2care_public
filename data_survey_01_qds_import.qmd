...
# 🔴 Note for Morri: We can probably delete all of this (or move it to the Wiki or something) after we know that the data is good. This will likely be unnecessary, irrelevant information to a new person reviewing this file for the first time once the data is good.
...
To ensure group assignment was consistent for subjects, we imported the subject, visit, and group assignments from the REDCap data. The remainder of the REDCap data would be utilized to supplement the QDS data at a later stage.

```{r}
redcap_spss_path <- here("data", "redcap", "All_Visits_Redcap.sav")
```

Import the data. Check the most recent file modification dates and print for user when this file is being sourced.
...
# 2023-09-28: REDCap imported with 119 rows and 3 columns.
```

## Excel Master Log

To ensure group assignment was consistent for subjects, we imported the subject and group assignments from the Excel Master log data. Group membership is randomly assigned at visit 2, but it is not recorded in QDS until visit 3 and beyond. Therefore, if we only rely on QDS data, it appears as though many participants haven't been randomized to a group even though they have. The study staff manually tracks group assignment in the Excel Master Log. We can use that data to cross-check and impute group membership when QDS and REDCap data are not otherwise sufficient. It is not ideal, but it seems to be the best option we have.
...
# 2023-09-28: Master log imported with 501 rows and 2 columns.
```
...
# 🔴 Note for Morri: I'm not sure what the comment below means. If I wrote it, then let's just delete it. If you wrote, please clarify.
...
# 🔴 Note for Morri: We use talk about "standardizing" the variables here and below. Do we ever define what that means? 
...
# 🔴 Note for Morri: It looks like you are taking a totally difference approach to this than I did, which is fine. I looks like your code is organized by section and then visit is nested under each section. I think this is a cool approach. However, it seems like the variable map is super important in this approach. I think we should write out definitions for each of the columns in the variable map.

# 🔴 Note for Morri: Notice how I changed the style of code lines that are longer than 80 characters in the code chunk below. Please see here: https://style.tidyverse.org/syntax.html#long-lines. Do you mind changing the style of the rest of the code chunks below, please?

```{r}
variable_map_vars <- c(
  'variable','section', 'sec_ord', 'instrument', 'inst_ord', 'var_ord', 'v1', 
  'v2', 'v3', 'v4', 'v5'
)

variable_map <- tibble::tibble(
  !!!variable_map_vars, .rows = 0, .name_repair = ~variable_map_vars
)
```
...
# 🔴 Note for Morri: There's a lot of repeated code above. Does this code work just as well?

```{r}
admin_col_name_terms <- c(
  "TODAY", "SUBJECT", "II", "VISIT", "TEST", "VISIT_TYPE", "ENDTIME", "ETIME",
  "CTIME", "GROUP"
)
vars <- v1 |> select(starts_with(admin_col_name_terms)) |> names()
vars
```
...
# 🔴 Note for Morri: My preference is for each code chunk to print no more than one value to the screen. The code chunk below prints three values to the screen, for example. Do you mind breaking it (and others like it) up, please? If you are just doint some sort of little data check, as I suspect you are below with sum(!is.na(v1$SUBJECT1)) and sum(!is.na(v1$TEST_V1)), you can simply type a comment like, "Data check: Count the number of non-missing values" or something like that.
...
We standardized these variable names. We add a missing value for a "group" variable in V1, as it was not present in the visit 1 data set. We added these values to the section variable map.

# 🔴 Note for Morri: I don't think I'm crazy about setting the item order with numbers like this. I can't look at this code and tell what order the variables are in. Let's chat about this. 
...
dr### Purging Temporary Containers
...
We checked for variables that were missing attributes after merging. We found 143 such variables.
...
We first attempted to apply attributes from the reordered V1 data set. There were 22 variables remaining that lacked attributes.
...
We next attempted to apply attributes from the reordered V2 data set. There were 5 variables remaining that lacked attributes.
