...
# 2023-04-19: qds_all_visits_import.rds imported as qds with 1502 rows and 1048 columns.
# 2023-04-19: redcap_import.rds imported as rc with 107 rows and 917 columns.
# 2023-04-19: master_log_screened_in.rds imported as ml with 442 rows and 55 columns.
# 2023-04-19: tlfb.rds imported as tlfb with 1768 rows and 21 columns.
# 2023-04-19: arrest.rds imported as arrest with 442 rows and 16 columns.
# 2023-04-19: bridge_wide.rds imported as bridge with 442 rows and 20 columns.
# 2023-04-19: ddt.rds imported as ddt with 1768 rows and 4 columns.
```
...
rc <- rc %>%
  ...
  mutate(
    vac_2 = as.numeric(vac_2)
  ) %>%
  mutate(
    vac_3 = as.numeric(vac_3)
  )
```
...
l2c_survey <- qds %>%
  ...
  mutate(
    sq_9l = as.numeric(sq_9l)
  ) %>%
  mutate(
    bh_24i = as.numeric(bh_24i)
  ) %>%
  mutate(
    vac_2 = as.numeric(vac_2)
  ) %>%
  ...
  rbind2(qds, rc, by("id", "visit"), all=TRUE)
```
...
```{r}
if (nrow(ml_sub) != length(unique(ml_sub$id))) {
  stop(
...
```{r}
if (length(unique(l2c_survey$id)) > length(unique(ml_sub$id))) {
  stop(
...
```{r}
l2c_survey <- ml_sub %>%
  left_join(l2c_survey, ml_sub, by = "id")
```
...
l2c_survey <- tlfb %>%
  ...
  left_join(l2c_survey, tlfb, by = "id")
...
l2c_survey <- arrest %>%
  left_join(l2c_survey, arrest, by = c("id"))
...
l2c_survey <- bridge %>%
  left_join(l2c_survey, bridge, by = c("id"))
...
l2c_survey <- ddt %>%
  left_join(l2c_survey, ddt, by = "id")
...
l2c_survey <- group_membership_key %>% 
  mutate_if(is.character,as.double) %>%
  select(-group) %>% 
  left_join(l2c_survey, by = "id") %>% 
