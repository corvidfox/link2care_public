...
# 2023-05-11: qds_all_visits_import.rds imported as qds with 1503 rows and 1048 columns.
# 2023-05-11: redcap_import.rds imported as rc with 108 rows and 659 columns.
# 2023-05-11: master_log_screened_in.rds imported as ml with 442 rows and 55 columns.
# 2023-05-11: tlfb.rds imported as tlfb with 1768 rows and 21 columns.
# 2023-05-11: arrest.rds imported as arrest with 442 rows and 16 columns.
# 2023-05-11: bridge_wide.rds imported as bridge with 442 rows and 20 columns.
# 2023-05-11: ddt.rds imported as ddt with 1768 rows and 4 columns.
```
...
```{r}
l2c_survey <- qds %>%
  mutate(vac_3 = as.character.Date(vac_3)) %>%
  bind_rows(rc) %>% 
  arrange(id, visit)
```

```{r}
dim(l2c_survey) # 1614 1048
```
...
```
## Remove columns with names/PHI
```{r}
l2c_survey <- select(l2c_survey, 1:5, 9:1129)
```

Make sure we get a warning if the number of columns in `l2c_survey` changes.

```{r}
if (ncol(l2c_survey) != 1126) {
  stop(
