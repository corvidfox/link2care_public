...
# 2021-04-25: QDS Visit 3 imported with 180 rows and 526 columns.
```
...
```{r}
if (!("col_name" %in% names(v3_cb_col_names))) {
  v3_cb_col_names[["col_name"]] <- NA
}
```
...
v3_cols_compare <- v3_cb_col_names %>% 
  ...
  select(cb_col_name_join, cb_col_name, col_name_v3 = col_name)
```

Prepare the previously established column names to be compared to the visit 3 column names.

```{r}
columns_cols_compare <- established_col_names %>% 
  # Don't need doc_index or section_name
  select(cb_col_name, col_name) %>% 
...
v3_col_names_key <- v3_cols_compare %>%
  left_join(columns_cols_compare, by = "cb_col_name_join") %>%
...
* Use the visit 3 column names for the following variables:
  ...
  - Weight-waist - fix typos in the visit 3 codebook   
  ...
  - HSI2_V13 - fix typo in the visit 3 codebook
...
```{r}
rm(
  col_name_keys, columns_cols_compare, established_col_names, v3_cb_col_names, 
...
```{r}
path <- "/Volumes/Link2Care/Participant Data/SPSS Data/QDS Data/v3.sav"
```
...
# 2021-04-25: QDS Visit 3 cleaned with 180 rows and 449 columns.
```
