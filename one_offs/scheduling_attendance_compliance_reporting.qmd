...
extract_values <- function(
    extracting_set, visit_list, date_rounding, date_windows
    ){
  ...
  for (visit_txt in visit_list){
    # Constants for visit: Number Scheduled, Number Attended
    col_head <- c(
    ...
    for (date_type in date_rounding){
    
      # Add Mean and Max days out of compliance, for date_type
      col <- c(col_head,
        round(unname(summary(
              abs(na.omit(extracting_set[[paste0(visit_txt, '_days', date_type)]]))
              )['Mean']), 2), # Mean days out of compliance
        trunc(round(unname(summary(
              abs(na.omit(extracting_set[[paste0(visit_txt, '_days', date_type)]]))
      ...
      for (window in date_windows){
        ...
        col <- c(col,
            trunc(sum(
                      !extracting_set[[paste0(visit_txt, window, date_type)]], 
                      na.rm = TRUE
                      ))
                 )
      }
      col_name = paste0(visit_txt, date_type)
      out_tibble[[col_name]] <- col
    }
  }
  out_tibble
}
