...
## REDCap 🔴 - PLACEHOLDER - POSSIBLY REMOVING
...
## Excel Master Log 🔴 - PLACEHOLDER - POSSIBLY REMOVING
...
Group membership is randomly assigned at visit 2, but it is not recorded in QDS until visit 3 and beyond.
Therefore, if we only rely on QDS data, it appears as though many participants haven't been randomized to a group even though they have.
...
master_log <- read_excel(
  ...
  col_types = c("text", rep("skip", 22), "text", rep("skip", 31)),
  skip = 1
) %>% 
...
# 2023-10-09: Master log imported with 501 rows and 2 columns.
...
# Create Initial Variable Map from QDS Sources
...
for (var in vars_v1){
  if(sum(!is.na(v1[,var])) == 0){
    print(paste(var, "is only missing values"))
  }
}

# "SUBJECT1 is only missing values"
# "TEST_V1 is only missing values"
```
...
We also set the desired order for the variables in this data set.
...
We set our initial desired standardization, including a point-fix to a variable.
...
# "subject"    "visit"      "visit_type" "today"      "ii"        
# "ctime"      "endtime_1" "etime"
```

We checked for any QDS variables that would either standardize differently, or were not present in the Visit 1 source data variables.
...
# Warning: Warning: The following column name(s) could not be standardized because it/they would have created a duplicate column name: II_V1, II_V2
  ...
  # Iterate through the variables, print message if all empty.
    for (
      ...
      if(sum(!is.na(source_df[,var])) == 0){
        print(paste(var,"in", source_name, "is only missing values"))
      }
    }
}

# [1] "SUBJECT1 in v1 is only missing values"
# [1] "TEST_V1 in v1 is only missing values"
# [1] "TEST_V2 in v2 is only missing values"
# [1] "TEST_V3 in v3 is only missing values"
# [1] "TEST_V5 in v5 is only missing values"
```

We added these 'TEST' variables to the list of variables to exclude.
We manually checked TEST_V4 in the QDS Visit 4 data set, and determined it should be dropped as well.

```{r}
dropping[['v2']] <- c(dropping[['v2']], 'TEST_V2')
dropping[['v3']] <- c(dropping[['v3']], 'TEST_V3')
dropping[['v4']] <- c(dropping[['v4']], 'TEST_V4')
dropping[['v5']] <- c(dropping[['v5']], 'TEST_V5')
```

We manually verified 'II_V1' and 'II_V2' in Visit 2, and determined 'II_V1' was an erroneous addition to this data set (and thus dropped).
The 'GROUP' variable was added from Visit 3 onward, and was desired.
...
```{r}
dropping[['v2']] <- c(dropping[['v2']], 'II_V1')
desired_vars <- append(desired_vars, 'group', 1)

desired_vars
# [1] "subject"    "group"      "visit"      "visit_type" "today"      "ii"         "ctime"      "endtime"    "etime" 
```
...
# [1] "ii_v1 is only missing values"
# [1] "visit_v1 is only missing values"
# [1] "macarthur_major_discrimination_timestamp is only missing values"
```

We dropped the variables that only contained missing values. We updated our desired variables to include the variables introduced in the REDCap data set, in our desired order.
...
desired_vars <- append(
  desired_vars, 
  c('maca_mdd_complete', 'ps_timestamp', 'ps_complete', 'email'), 
  length(desired_vars)
  )

desired_vars
#  [1] "subject"           "group"             "visit"            
#  [4] "redcap_event_name" "redcap_event_name" "visit_type"       
#  [7] "today"             "ii"                "ctime"            
# [10] "endtime"           "etime"             "maca_mdd_complete"
# [13] "ps_timestamp"      "ps_complete"       "maca_mdd_complete"
# [16] "ps_timestamp"      "ps_complete"       "email"   
```
...
As the REDCap variable naming was significantly different from the QDS variable naming, we manually created our source map for this set.
...
source_maps <- vm_make_source_map(
  ...
  full_join(tibble::tibble(
    ...
    'variable' = c(
        'ii', 'group', 'visit', 'redcap_event', 'today', 
        'subject', 'email',
        'maca_mdd_complete',
        'ps_timestamp', 
        'ps_complete'
...
for (var in vars_v1){
  if(sum(!is.na(v1[,var])) == 0){
    print(paste(var, "is only missing values"))
    missing_val_vars <- c(missing_val_vars, var)
  }
}
...
# [1] "sq_2 is only missing values"
# [1] "sq_3 is only missing values"
# [1] "sq_4 is only missing values"
# [1] "sq_5 is only missing values"
# [1] "sq_6 is only missing values"
# [1] "sq_7 is only missing values"
# [1] "sq_7a is only missing values"
# [1] "sq_7b1 is only missing values"
# [1] "sq_7b2 is only missing values"
# [1] "sq_7d is only missing values"
# [1] "sq_8 is only missing values"
# [1] "sq_9 is only missing values"
# [1] "sq_9l is only missing values"
# [1] "sq_10 is only missing values"
# [1] "sq_11 is only missing values"
# [1] "sq_12 is only missing values"
# [1] "sq_13 is only missing values"
# [1] "sq_14 is only missing values"
# [1] "sq_15 is only missing values"
# [1] "sq_16 is only missing values"
# [1] "sq_17 is only missing values"
# [1] "sq_19 is only missing values"
# [1] "sq_20 is only missing values"
# [1] "sq_21 is only missing values"
# [1] "sq_23 is only missing values"
# [1] "gender is only missing values"
```
...
source_maps <- vm_make_source_map(
  ...
  full_join(vm_make_source_map(
    ...
    mutate(variable = case_when(
      ...
      variable == 'sq_18___0' ~ 'sq_18a',
      variable == 'sq_18___1' ~ 'sq_18b',
      variable == 'sq_18___2' ~ 'sq_18c',
      variable == 'sq_18___3' ~ 'sq_18d',
      variable == 'sq_18___4' ~ 'sq_18e',
      variable == 'sq_18___5' ~ 'sq_18f',
      variable == 'sq_18___6' ~ 'sq_18g',
      variable == 'sq_18___7' ~ 'sq_18h',
      variable == 'sq_18___8' ~ 'sq_18i',
      variable == 'sq_22___0' ~ 'sq_22a',
      variable == 'sq_22___1' ~ 'sq_22b',
      variable == 'sq_22___2' ~ 'sq_22c',
      variable == 'sq_22___3' ~ 'sq_22d',
      variable == 'sq_22___4' ~ 'sq_22e',
      variable == 'sq_22___5' ~ 'sq_22f',
      variable == 'sq_22___6' ~ 'sq_22g',
      variable == 'sq_22___7' ~ 'sq_22h',
      TRUE ~ variable
    )
  ),
  by = "variable"
...
# [1] "mms_1a is only missing values"
# [1] "mms_1b is only missing values"
# [1] "mms_1c is only missing values"
# [1] "mms_1d is only missing values"
# [1] "mms_1e is only missing values"
# [1] "mms_2a is only missing values"
# [1] "mms_2b is only missing values"
# [1] "mms_2c is only missing values"
# [1] "mms_2d is only missing values"
# [1] "mms_2e is only missing values"
# [1] "mms_7 is only missing values"
# [1] "mms_9 is only missing values"
# [1] "mms_10 is only missing values"
# [1] "mms_11 is only missing values"
```
...
source_maps <- vm_make_source_map(
  ...
  full_join(
    ...
    mutate(variable = case_when(
      variable == 'mms_3___1' ~ 'mms_3g',
      variable == 'mms_3___2' ~ 'mms_3h',
      variable == 'mms_3___3' ~ 'mms_3i',
      variable == 'mms_3___0' ~ 'mms_3j',
      variable == 'mms_4___1' ~ 'mms_4a',
      variable == 'mms_4___2' ~ 'mms_4b',
      variable == 'mms_4___3' ~ 'mms_4c',
      variable == 'mms_4___4' ~ 'mms_4d',
      variable == 'mms_4___5' ~ 'mms_4e',
      variable == 'mms_4___0' ~ 'mms_4f',
      variable == 'mms_4v___1' ~ 'mms_4va',
      variable == 'mms_4v___2' ~ 'mms_4vb',
      variable == 'mms_4v___3' ~ 'mms_4vc',
      variable == 'mms_4v___4' ~ 'mms_4vd',
      variable == 'mms_4v___5' ~ 'mms_4ve',
      variable == 'mms_4v___0' ~ 'mms_4vf',
      variable == 'mms_5___1' ~ 'mms_5a',
      variable == 'mms_5___2' ~ 'mms_5b',
      variable == 'mms_5___3' ~ 'mms_5c',
      variable == 'mms_5___0' ~ 'mms_5d',
      variable == 'mms_6___1' ~ 'mms_6a',
      variable == 'mms_6___2' ~ 'mms_6b',
      variable == 'mms_6___0' ~ 'mms_6c',
      variable == 'mms_8___1' ~ 'mms_8a',
      variable == 'mms_8___2' ~ 'mms_8b',
      variable == 'mms_8___3' ~ 'mms_8c',
      variable == 'mms_8___0' ~ 'mms_8d',
      TRUE ~ variable
      )
    ),
  by = "variable"
...
source_maps <- vm_make_source_map(
  ...
  full_join(
    ...
    mutate(variable = case_when(
      variable == 'realm___0' ~ 'realma',
      variable == 'realm___1' ~ 'realmb',
      variable == 'realm___2' ~ 'realmc',
      variable == 'realm___3' ~ 'realmd',
      variable == 'realm___4' ~ 'realme',
      variable == 'realm___5' ~ 'realmf',
      variable == 'realm___6' ~ 'realmg',
      variable == 'realm___7' ~ 'realmh',
      variable == 'realm___8' ~ 'realmi',
...
# [1] "read1_v1 is only missing values"
# [1] "read2_v1 is only missing values"
# [1] "read3_v5 is only missing values"
```
...
# [6] "In source  redcap : read1_v1, read2_v1, read3_v5"     
```
...
source_maps <- vm_make_source_map(
  ...
  full_join(
    ...
    mutate(variable = case_when(
      variable == 'read_1_v1' ~ 'read_1',
      variable == 'read_2_v1' ~ 'read_2',
      variable == 'read_3_v5' ~ 'read_3',
      TRUE ~ variable
      )
    ),
  by = "variable"
...
# [1] "DEM13EV1 is only missing values"
# [1] "DEM14EV1 is only missing values"
```
...
# [1] "dem13av1 is only missing values"
# [1] "dem13bv1 is only missing values"
# [1] "dem13cv1 is only missing values"
# [1] "dem13dv1 is only missing values"
# [1] "dem13ev1 is only missing values"
# [1] "dem13fv1 is only missing values"
# [1] "dem13gv1 is only missing values"
# [1] "dem13hv1 is only missing values"
# [1] "dem14dv1 is only missing values"
# [1] "dem14ev1 is only missing values"
# [1] "dem14fv1 is only missing values"
# [1] "dem14gv1 is only missing values"
```
...
# [6] "In source  redcap : dem1v1, dem2v1, dem3v1, dem4v1___1, dem4v1___2, 
# dem4v1___3, dem4v1___4, dem4v1___5, dem4v1___6, dem5v1, dem5av1, dem6v1, 
# dem6av1, dem7v1___1, dem7v1___2, dem7v1___3, dem7v1___4, dem7v1___5, dem8v1,
# dem9v1, dem10v1, dem11v1, dem12v1___1, dem12v1___2, dem12v1___3, 
# dem12v1___4, dem12v1___5, dem12v1___6, dem12v1___7, dem12v1___8, 
# dem12v1___9, dem12v1___10, dem12v1___11, dem12v1___12, dem13v1, dem13av1, 
# dem13bv1, dem13cv1, dem13dv1, dem13ev1, dem13fv1, dem13gv1, dem13hv1, 
# dem14v1, dem14av1, dem14bv1, dem14cv1, dem14dv1, dem14ev1, dem14fv1, 
# dem14gv1, dem15v1, dem16v1, dem17v1, dem18v1, dme6bv3"
```
...
source_maps <- vm_make_source_map(
  ...
  full_join(
    ...
    mutate(variable = case_when(
      variable == 'dem_1v1' ~ 'dem_1',
      variable == 'dem_2v1' ~ 'dem_2',
      variable == 'dem_3v1' ~ 'dem_3',
      variable == 'dem_4v1___1' ~ 'dem_4a',
      variable == 'dem_4v1___2' ~ 'dem_4b',
      variable == 'dem_4v1___3' ~ 'dem_4c',
      variable == 'dem_4v1___4' ~ 'dem_4d',
      variable == 'dem_4v1___5' ~ 'dem_4e',
      variable == 'dem_4v1___6' ~ 'dem_4f',
      variable == 'dem_5v1' ~ 'dem_5',
      variable == 'dem_5av1' ~ 'dem_5a',
      variable == 'dem_6v1' ~ 'dem_6',
      variable == 'dem_6av1' ~ 'dem_6a',
      variable == 'dme_6bv3' ~ 'dme_6b',
      variable == 'dem_7v1___1' ~ 'dem_7a',
      variable == 'dem_7v1___2' ~ 'dem_7b',
      variable == 'dem_7v1___3' ~ 'dem_7c',
      variable == 'dem_7v1___4' ~ 'dem_7d',
      variable == 'dem_7v1___5' ~ 'dem_7e',
      variable == 'dem_8v1' ~ 'dem_8',
      variable == 'dem_9v1' ~ 'dem_9',
      variable == 'dem_10v1' ~ 'dem_10',
      variable == 'dem_11v1' ~ 'dem_11',
      variable == 'dem_12v1___1' ~ 'dem_12a',
      variable == 'dem_12v1___2' ~ 'dem_12b',
      variable == 'dem_12v1___3' ~ 'dem_12c',
      variable == 'dem_12v1___4' ~ 'dem_12d',
      variable == 'dem_12v1___5' ~ 'dem_12e',
      variable == 'dem_12v1___6' ~ 'dem_12f',
      variable == 'dem_12v1___7' ~ 'dem_12g',
      variable == 'dem_12v1___8' ~ 'dem_12h',
      variable == 'dem_12v1___9' ~ 'dem_12i',
      variable == 'dem_12v1___10' ~ 'dem_12j',
      variable == 'dem_12v1___11' ~ 'dem_12k',
      variable == 'dem_12v1___12' ~ 'dem_12l',
      variable == 'dem_13v1' ~ 'dem_13',
      variable == 'dem_13av1' ~ 'dem_13a',
      variable == 'dem_13bv1' ~ 'dem_13b',
      variable == 'dem_13cv1' ~ 'dem_13c',
      variable == 'dem_13dv1' ~ 'dem_13d',
      variable == 'dem_13ev1' ~ 'dem_13e',
      variable == 'dem_13fv1' ~ 'dem_13f',
      variable == 'dem_13gv1' ~ 'dem_13g',
      variable == 'dem_13hv1' ~ 'dem_13h',
      variable == 'dem_14v1' ~ 'dem_14',
      variable == 'dem_14av1' ~ 'dem_14a',
      variable == 'dem_14bv1' ~ 'dem_14b',
      variable == 'dem_14cv1' ~ 'dem_14c',
      variable == 'dem_14dv1' ~ 'dem_14d',
      variable == 'dem_14ev1' ~ 'dem_14e',
      variable == 'dem_14fv1' ~ 'dem_14f',
      variable == 'dem_14gv1' ~ 'dem_14g',
      variable == 'dem_15v1' ~ 'dem_15',
      variable == 'dem_16v1' ~ 'dem_16',
      variable == 'dem_17v1' ~ 'dem_17',
      variable == 'dem_18v1' ~ 'dem_18',
      TRUE ~ variable
      )
    ),
  by = "variable"
...
# [6] "In source  redcap : bh1v1y, bh1v1m, bh1v1d, bh2v1, bh3v1, bh4v1y, 
# bh4v1m, bh4v1d, bh5v1, bh6v1, bh7v1y, bh7v1m, bh7v1w, bh8v1, bh9v1, bh10v1, 
# bh11av1, bh12av1___1, bh12av1___2, bh12av1___3, bh12av1___4, bh12av1___5, 
# bh12av1___6, bh12av1___7, bh12av1___8, bh12av1___9, bh12av1___10, 
# bh12av1___11, bh12av1___12, bh13av1___1, bh13av1___2, bh13av1___3, 
# bh13av1___4, bh13av1___5, bh13av1___6, bh13bv1___1, bh13bv1___2, 
# bh13bv1___3, bh13bv1___4, bh13bv1___5, bh13bv1___6, bh13bv1___7, 
# bh13bv1___8, bh13cv1___1, bh13cv1___2, bh13cv1___3, bh13cv1___4, 
# bh13cv1___5, bh13cv1___6, bh13cv1___7, bh13cv1___8, bh14v1, bh14bv1y, 
# bh14bv1m, bh14bv1w, bh14bv3d, bh15v1, bh15av1, bh16v1, bh17v1y, bh17v1m, 
# bh17v1w, bh18v1, bh18bv1___1, bh18bv1___2, bh18bv1___3, bh18bv1___4, 
# bh18bv1___5, bh18bv1___6, bh18bv1___7, bh18bv1___8, bh18b1v1, bh19v1, bh20v"
```
...
```{r}
desired_vars <- append(
  desired_vars, 'bh_14_bd', after=which(desired_vars == 'bh_14bw')
  )
...
# [49] "bh_13bh"  "bh_13c"   "bh_13ca"  "bh_13cb"  "bh_13cc"  "bh_13cd" 
# [55] "bh_13ce"  "bh_13cf"  "bh_13cg"  "bh_13ch"  "bh_14"    "bh_14b"  
# [61] "bh_14by"  "bh_14bm"  "bh_14bw"  "bh_14_bd" "bh_15"    "bh_15a"  
...
source_maps <- vm_make_source_map(
  ...
  vm_make_source_map(
    ...
    mutate(variable = case_when(
      variable == 'bh_1v1y' ~ 'bh_1y',
      variable == 'bh_1v1m' ~ 'bh_1m',
      variable == 'bh_1v1d' ~ 'bh_1d',
      variable == 'bh_2v1' ~ 'bh_2',
      variable == 'bh_3v1' ~ 'bh_3',
      variable == 'bh_4v1y' ~ 'bh_4y',
      variable == 'bh_4v1m' ~ 'bh_4m',
      variable == 'bh_4v1d' ~ 'bh_4d',
      variable == 'bh_5v1' ~ 'bh_5',
      variable == 'bh_6v1' ~ 'bh_6',
      variable == 'bh_7v1y' ~ 'bh_7y',
      variable == 'bh_7v1m' ~ 'bh_7m',
      variable == 'bh_7v1w' ~ 'bh_7w',      
      variable == 'bh_8v1' ~ 'bh_8',
      variable == 'bh_9v1' ~ 'bh_9',
      variable == 'bh_10v1' ~ 'bh_10',
      variable == 'bh_11av1' ~ 'bh_11a',
      variable == 'bh_12av1___1' ~ 'bh_12aa',
      variable == 'bh_12av1___2' ~ 'bh_12ab',
      variable == 'bh_12av1___3' ~ 'bh_12ac',
      variable == 'bh_12av1___4' ~ 'bh_12ad',
      variable == 'bh_12av1___5' ~ 'bh_12ae',
      variable == 'bh_12av1___6' ~ 'bh_12af',
      variable == 'bh_12av1___7' ~ 'bh_12ag',
      variable == 'bh_12av1___8' ~ 'bh_12ah',
      variable == 'bh_12av1___9' ~ 'bh_12ai',
      variable == 'bh_12av1___10' ~ 'bh_12aj',
      variable == 'bh_12av1___11' ~ 'bh_12ak',
      variable == 'bh_12av1___12' ~ 'bh_12al',
      variable == 'bh_13av1___1' ~ 'bh_13aa',
      variable == 'bh_13av1___2' ~ 'bh_13ab',
      variable == 'bh_13av1___3' ~ 'bh_13ac',
      variable == 'bh_13av1___4' ~ 'bh_13ad',
      variable == 'bh_13av1___5' ~ 'bh_13ae',
      variable == 'bh_13av1___6' ~ 'bh_13af',
      variable == 'bh_13bv1___1' ~ 'bh_13ba',
      variable == 'bh_13bv1___2' ~ 'bh_13bb',
      variable == 'bh_13bv1___3' ~ 'bh_13bc',
      variable == 'bh_13bv1___4' ~ 'bh_13bd',
      variable == 'bh_13bv1___5' ~ 'bh_13be',
      variable == 'bh_13bv1___6' ~ 'bh_13bf',
      variable == 'bh_13bv1___7' ~ 'bh_13bg',
      variable == 'bh_13bv1___8' ~ 'bh_13bh',
      variable == 'bh_13cv1___1' ~ 'bh_13ca',
      variable == 'bh_13cv1___2' ~ 'bh_13cb',
      variable == 'bh_13cv1___3' ~ 'bh_13cc',
      variable == 'bh_13cv1___4' ~ 'bh_13cd',
      variable == 'bh_13cv1___5' ~ 'bh_13ce',
      variable == 'bh_13cv1___6' ~ 'bh_13cf',
      variable == 'bh_13cv1___7' ~ 'bh_13cg',
      variable == 'bh_13cv1___8' ~ 'bh_13ch',
      variable == 'bh_14v1' ~ 'bh_14',
      variable == 'bh_14bv1y' ~ 'bh_14by',
      variable == 'bh_14bv1m' ~ 'bh_14bm',
      variable == 'bh_14bv1w' ~ 'bh_14bw',
      variable == 'bh_14bv3d' ~ 'bh_14_bd',
      variable == 'bh_15v1' ~ 'bh_15',
      variable == 'bh_15av1' ~ 'bh_15a',
      variable == 'bh_16v1' ~ 'bh_16',
      variable == 'bh_17v1y' ~ 'bh_17y',
      variable == 'bh_17v1m' ~ 'bh_17m',
      variable == 'bh_17v1w' ~ 'bh_17w',
      variable == 'bh_18v1' ~ 'bh_18',
      variable == 'bh_18bv1___1' ~ 'bh_18ba',
      variable == 'bh_18bv1___2' ~ 'bh_18bb',
      variable == 'bh_18bv1___3' ~ 'bh_18bc',
      variable == 'bh_18bv1___4' ~ 'bh_18bd',
      variable == 'bh_18bv1___5' ~ 'bh_18be',
      variable == 'bh_18bv1___6' ~ 'bh_18bf',
      variable == 'bh_18bv1___7' ~ 'bh_18bg',
      variable == 'bh_18bv1___8' ~ 'bh_18bh',
      variable == 'bh_18b1v1' ~ 'bh_18b1',
      variable == 'bh_19v1' ~ 'bh_19',
...
We manually verified these variables and determined the `well_being` values were separate but related questions added after Visit 1. We also found the REDCap variables escaped our standardization function.
...
# [6] "In source  redcap : sss1_v1, sss2_v1, well_being1, well_being2, 
# well_being3"
```
...
source_maps <- vm_make_source_map(
  ...
  full_join(
    ...
    mutate(variable = case_when(
      variable == 'sss_1_v1' ~ 'sss_1',
      variable == 'sss_2_v1' ~ 'sss_2',
      TRUE ~ variable
      )
    ),
  by = "variable"
...
We identified our keys and identified the matching variables in the visit 1 set.
...
We checked to see if any of these variables were only missing values in Visit 1, and none were.
...
dropping[['v3']] <- c(
  ...
  'PHQ7_V3_dichot', 'PHQ8_V3_dichot', 'PHQ_dep_dichot_total', 
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set.
The only variables found were the calculated variables in the other visit data sets.
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set, and found none.
...
# Must be a "non-empty string", but no valid variable name is a single space
excl_keys <- list(
  'starts_with' = c("SQ", "SSS", "SUBJECT", "SUBSTANCE", "SPOUSE", "SB"),
  'contains' = c(" "))
...
We identified our keys and identified the matching variables in the visit 2 set.
...
We checked for any variables that would either standardize differently, or were absent from the visit 2 source data set.
...
# [1] "In source  v1 : "                
# [2] "In source  v2 : DS10_V2, DS11_V2"
# [3] "In source  v3 : "                
# [4] "In source  v4 : "                
# [5] "In source  v5 : " 
```
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set.
We only identified the SPSS calculated variables previously identified for dropping in Visit 1.
...
We checked for any variables that would either standardize differently, or were absent from the visit 2 source data set, and found none.
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set, and found none.
We only identified the variables which escaped our standardization function, and the previously identified SPSS variables to drop.
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set.
We identified the SPSS calculated variables identified for dropping, but also a typographical error that caused a variable to escape our standardization function.
...
# [2] "In source  v2 : "                            
# [3] "In source  v3 : HSI2_V13, hsi_V3, HSI_V3_cat"
# [4] "In source  v4 : HSI2_V14, hsi_V4, HSI_V4_cat"
...
### Alcohol and Drug Timeline Follow-Back (TLFB) 🔴 - PLACEHOLDER
...
In manual inspection of the variables, we identified that the variable `pbq_v2_total` was an SPSS calculated variable only present in Visit 2, which we wished to drop.
...
# [1] "In source  v1 : "                
# [2] "In source  v2 : FSS1_V2, FSS2_V2, FSS3_V2, FSS3A_V2, FSS4_V2, FSS5_V2, fss_a_V2, fss_b_V2, fss_c_V2, fss_d_V2, fss_e_V2, fss_f_V2, FSS_V2_total, FSS_V2_score, FSS_V2_score_dichotomous"
# [3] "In source  v3 : FSS1_V3, FSS2_V3, FSS3_V3, FSS3A_V3, FSS4_V3, FSS5_V3, fss_a_V3, fss_b_V3, fss_c_V3, fss_d_V3, fss_e_V3, fss_f_V3, FSS_V3_total, FSS_V3_score, FSS_V3_score_dichotomous"
# [4] "In source  v4 : FSS1_V4, FSS2_V4, FSS3_V4, FSS3A_V4, FSS4_V4, FSS5_V4, fss_a_V4, fss_b_V4, fss_c_V4, fss_d_V4, FSSe_V4, FSSf_V4, FSS_V4_total, FSS_V4_score, FSS_V4_score_dichotomous"  
# [5] "In source  v5 : FSS1_V5, FSS2_V5, FSS3_V5, FSS3A_V5, FSS4_V5, FSS5_V5, fss_a_V5, fss_b_V5, fss_c_V5, fss_d_V5, fss_e_V5, fss_f_V5, FSS_V5_total, FSS_V5_score, FSS_V5_score_dichotomous"
```
...
We checked for any variables that would either standardize differently, or were absent from the visit 1 source data set.
We only identified the variable with a typographical error in Visit 1, which had a similar error in Visit 5.
...
We identified our keys and identified the matching variables in the visit 2 set.MacAurthur Major Discrimination was not used in QDS Visit 1.
...
In manual inspection of the variables, we identified that the variables `uls_V2_total`, `uls_V3_total`, `uls_V4_total`, and `uls_V5_total` were a SPSS calculated variables that we wished to drop.
...
# PV2_V2, PV3_V2
# Warning: Warning: The following column name(s) could not be standardized 
# because it/they would have created a duplicate column name: PV1_V3, 
# PV2_V3
# Warning: Warning: The following column name(s) could not be standardized 
# because it/they would have created a duplicate column name: PV1_V4, 
# PV2_V4
# Warning: Warning: The following column name(s) could not be standardized 
# because it/they would have created a duplicate column name: PV1_V5, 
# PV2_V5, PV3_V5
# [1] "In source  v1 : "                      
...
We identified our keys and identified the matching variables in the visit 3 set.
...
We checked for any variables that would either standardize differently, or were absent from the visit 3 source data set, and found none.
...
# Verifying Remaining Variables 🔴 - Placeholder as of 2023-10-06

We examined the 68 variables that had otherwise failed to be assigned from each QDS data set. These variables were all determined to be SPSS calculated variables we wished to recreate in R from our aggregated set.
...
#  [1] "TEST_V1"                 "SUBJECT1"                "BRIS2_QD_dichot"         "QD_unintent_slp"        
#  [5] "PA"                      "VA"                      "A"                       "HA"                     
#  [9] "AlcQF_V1"                "binge_drinker_V1"        "Heavy_drinker"           "current_months_homeless"
# [13] "friends_smoke_V1"        "spouse_smoke"            "Mental_Health_dx"        "pain"                   
# [17] "Substance_abuse_hs"      "Substance_use_30d"       "Any_health_insurance"    "Time_to_complete_V1"    
# [21] "TCU_DS_TOTAL_V2"         "TCU_DS_SCORE_V2"         "TCU_desire_v2_total"     "TCU_tn_v2_total"        
# [25] "TCU_ts_v2_total"         "TCU_se_v2_total"         "TCU_hs_v2_total"         "TCU_rt_v2_total"        
# [29] "Time_to_complete_V2"     "BH14BV3D"                "PHQ_dep_dichot_total_V3" "BRIS2_V3_dichot"        
# [33] "unintent_slp_V3"         "AlcQF_V3"                "binge_drinker_V3"        "Heavy_drinker_V3"       
# [37] "PA_V3"                   "VA_V3"                   "A_V3"                    "HA_V3"                  
# [41] "Time_to_complete_V3"     "BH14BV4D"                "PHQ_dep_dichot_total_V4" "BRIS2_V4_dichot"        
# [45] "unintent_slp_V4"         "AlcQF_V4"                "binge_drinker_V4"        "Heavy_drinker_V4"       
# [49] "TCU_desire_v4_total"     "TCU_tn_v4_total"         "TCU_ts_V4_total"         "PA_V4"                  
# [53] "VA_V4"                   "A_V4"                    "HA_V4"                   "Time_to_complete_V4"    
# [57] "BH14BV5D"                "PHQ_dep_dichot_total_V5" "BRIS2_V5_dichot"         "unintent_slp_V5"        
# [61] "AlcQF_V5"                "binge_drinker_V5"        "Heavy_drinker_V5"        "PA_V5"                  
# [65] "VA_V5"                   "A_V5"                    "HA_V5"                   "Time_to_complete_V5"
```
