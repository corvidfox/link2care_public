...
This file is used to generate the organizational map linking Link2Care follow-up visit source data variables to the final combined data set. Identification of desired variables, linkage of source data variables that capture identical data, and organization of variables into a logical order are achieved. SPSS calculated variables were dropped in favor of recreation in R, using the processed, combined data set. 

Further data cleaning and wrangling occur in later files. The product of this file is the initial Variable Map.
...
1.  This file creates a **variable map**.
    This variable map is used to facilitate batch processing of the Link2Care variables. Specifically, variables from the QDS, REDCap, Timeline Follow Back, and Delay Discounting Task were imported and mapped. Limited variables from the Master Log (subject ID, group assignment, and age) were also imported for mapping.
...
**2023-10-13**, Added attributes (label and labels) comparison and capture in variable map. **This file should be complete.**

**2023-10-11**, REDCap, TLFB, Master Log, and DDT variables added. Staff Perceptions of Link2Care, EMA data is not present.
...
We identified the Delayed Discount Task (DDT) variable appropriate for this section.
...
As the REDCap, Master Log, and TLFB variable naming was significantly different from the QDS variable naming, we manually created our source map for these sets. The single variable from the Delayed Discount Task (DDT) was added manually for brevity.
...
We added the section to our overall list. We purged the instrument and section maps, as well as the Master Log data frame to reduce memory burden in processing and protect against potential memory leak.
...
Screening was performed with 5 instruments: Screening Questionnaire, Mini-Mental State Exam, Rapid Estimate of Adult Literacy in Medicine (REALM), BiologicalMeasures/Anthropometrics, and a Locator Form. The Locator Form was excluded from analysis.
...
We set our initial desired order and standardization. We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates.
...
We added the instrument to our section-wise list of instruments. We purged the instrument map to reduce memory burden in processing and protect against potential memory leak.
...
We identified the variables present in the DDT data set. These variables were not labeled in SPSS. The data was in a wide format, where each subject's responses were recorded in a single row. This would be managed with later processing.
...
We set our initial desired order and standardization. We utilized tidyselect to enable a warning/error message if the variable name changes after our initial processing, to facilitate updates. Due to the format of these variables, our standardization function would fail. As such, we performed manual standardization.

```{r}
ddt_vars <- ddt %>%
  select(all_of(c(
    "K_V1", "ED50_V1", "K_V3", "ED50_V3", "K_V4", "ED50_V4", 
    "K_V5", "ED50_V5"
    ))
  ) %>%
  names()
...
desired_vars <- c(
  "ddt_k_v1", "ddt_ed50_v1", "ddt_k_v3", "ddt_ed50_v3",
  "ddt_k_v4", "ddt_ed50_v4", "ddt_k_v5", "ddt_ed50_v5"
  )

desired_vars

# [1] "ddt_k_v1"    "ddt_ed50_v1" "ddt_k_v3"    "ddt_ed50_v3" "ddt_k_v4"    
# [6] "ddt_ed50_v4" "ddt_k_v5"    "ddt_ed50_v5"
```
...
We renamed our data sets to facilitate easier use of dplyr and tidyverse functions. We added a "qds" prefix to the QDS visit columns in our variable map.
...
We extracted and compared the labels for each variable in our source data sets. Variables with more than one label across the data sets were isolated for manual review.
...
We determined that there were 3 variables (`bh_14`, `t_10`, and `dd_10`) with significantly different labels. The remaining variables had slight punctuation differences which did not change the meaning of the label.

We chose the QDS V1 labels to form our initial labels. Other data sets would be utilized if QDS V1 was empty.
...
We repeated a similar process for the variable value labels. These value labels were absent from the REDCap, TLFB, and DDT data sets.
...
We chose the QDS V1 variable value labels to form our initial variable value labels. Other data sets would be utilized if QDS V1 was empty.
...
variable_map_path <- here(
  "data", "Combined Participant Data", "variable_map.xlsx"
  )
