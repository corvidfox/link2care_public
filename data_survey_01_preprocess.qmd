...
# 2023-12-13: Master log imported with 442 rows and 19 columns.
...
spss_path <- here::here("data", "master_log", "ml_protocol_dates.sav")
```
...
```{r}
ddt_spss_path <- here::here("data", "ddt", "L2C_DDT_Database_1.sav")
```
...
# 2023-12-13: DDT imported with 442 rows and 9 columns.
...
data_stats <- retrieve_data_stats('ddt_orig_stats')
...
# ℹ In argument: `dplyr::across(everything(), as.numeric)`.
# Caused by warning:
...
# MB also wants an SPSS version
spss_path <- here::here("data", "ddt", "l2c_ddt_database_2_long.sav")
```
...
```{r}
arrest_path <- here::here("data", "arrest", "arrest.xlsx")
```
...
# 2023-12-13 : Arrest data imported with 442 rows and 16 columns.
```
...
data_stats <- retrieve_data_stats('arrest_orig_stats')

data_mod_check(
  'df' = arrest_data,
  'df_path_str' = arrest_path,
  'orig_path_str' = data_stats$xlsx_path,
  'mod_dt' = data_stats$mod_dt,
  'num_rows' = data_stats$num_rows,
  'num_cols' = data_stats$num_cols,
  'col_names' = data_stats$cols
...
We eliminated subjects that were missing a baseline date to eliminate any extra rows of empty values. Subjects that had arrest dates recorded before the baseline date, after the 1 year follow up period end date, and out of chronological order were verified with JB on 2023-12-11.
...
arrest_data <- arrest_data |>
  ...
  # Calculate the number of arrests that occurred in the study period (V2-V5)
    v2_v5_total = sum(dplyr::c_across(v1_v2_count:v4_v5_count)),
  # Calculate the number of arrests that occurred in the follow-up period 
  # (V2-end)
    v2_end_total = sum(dplyr::c_across(v1_v2_count:v5_end_count))
  ) |>
  dplyr::ungroup() |>
...
attributes(arrest_data$v1_v2_count)$label <- paste0(
  "Number of recorded arrests between Baseline and V2"
  )
attributes(arrest_data$v2_v3_count)$label <- paste0(
  "Number of recorded arrests between V2 and V3 protocol dates"
  )
attributes(arrest_data$v3_v4_count)$label <- paste0(
  "Number of recorded arrests between V3 and V4 protocol dates"
  )
attributes(arrest_data$v4_v5_count)$label <- paste0(
  "Number of recorded arrests between V4 and V5 protocol dates"
  )
attributes(arrest_data$v5_end_count)$label <- paste0(
...
### 💾 Save the Processed Arrest Data
...
# MB also wants an SPSS version
spss_path <- here::here("data", "arrest", "arrest_1.sav")
```
...
# 2023-08-25 10:43:48 
#  2023-12-13 : Bridge Session data imported with 4377 rows and 10 columns.
```
...
#  2023-12-13 : Bridge Session point-fix data imported with 25 rows and 
#  3 columns.
```
...
data_stats <- retrieve_data_stats('bridge_orig_stats')
...
data_stats <- retrieve_data_stats('bridge_pointfix_stats')
...
bridge_fixes <- bridge_fixes |>
  dplyr::rename_at('original_date', ~'bridge_date_session')
...
### 💾 Save the Processed Bridge Session Data
