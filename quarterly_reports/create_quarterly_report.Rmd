...
# 2022-08-14: Master log screened in imported with 405 rows and 55 columns.
```
...
# 2022-08-14: Master log screened out imported with 64 rows and 12 columns.
```
...
# Errors are because some zeros are stored as text instead of numbers in the Excel file. Safe to ignore.
# For data checking
# select(ema, id, emas_9) %>% filter(id == 2111)
```
...
## Fill in Table 3. Payment card and phone loss
purrr::walk(
  .x = list("tot_clin_card_dist"),
  .f = ~ body_replace_text_at_bkm(doc, .x, as.character(get(.x)))
)
...
```{r}
doc <- read_docx("template_l2c_quarterly_report.docx")
for (val in c("tot_clin_card_dist")) {
  doc <- doc %>% 
    body_replace_text_at_bkm(val, as.character(get(val)))
}
```

```{r}
t3_n_clincard_replacements_ft <- t3_n_clincard_replacements %>% 
  select(n) %>% 
  slice(-1) %>% 
  flextable() %>% 
  delete_part("header") %>% 
...
doc <- read_docx("template_l2c_quarterly_report.docx")
doc <- doc %>% 
  body_replace_flextable_at_bkm("t3_n_clincard_replacements", t3_n_clincard_replacements_ft)
```
