...
# 2021-07-09: QDS Visit 2 imported with 271 rows and 210 columns.
```
...
On 2021-06-10, VISIT_TYPE was added to QDS, but not the codebook. Therefore, we will create a new row for v2_cb_col_names that will include the VISIT_TYPE variables that would have been there if it were in the codebook. 

```{r}
v2_cb_col_names <- v2_cb_col_names %>%
  add_row(
    # To help with nesting it in the correct place in the current variable list
    doc_index    = NA,
    section_name = "Admin",
    cb_col_name  = "VISIT_TYPE",
    col_name     = "visit_type",
    .after       = which(v2_cb_col_names$cb_col_name == "TODAY_V2")
  )
```

## New columns: COVID vaccine 

On 2021-03-08, questions about COVID vaccine were added to QDS, but not the codebook. Therefore, we will create new rows for v2_cb_col_names that will add the COVID variables that would have been there if the new variables were in the codebook. 
...
v2_cb_col_names <- v2_cb_col_names %>%
  add_row(
    ...
    col_name     = c(
      "vac_1", "vac_2", "vac_2y", "vac_2m", "vac_2d", "vac_3", "vac_3y", 
      "vac_3m", "vac_3d"
    )
  )
```
...
II_V1 shouldn't be in this data frame. Drop it.

```{r}
in_df_not_cb <- in_df_not_cb[in_df_not_cb != "II_V1"]
```
...
* There were 9 COVID vaccine questions added around 2021-03-08. 
* Visit type was added to QDS on 2021-06-15.
* II_V1 shouldn't be in this data frame. Drop it.
...
After the above adjustments, the expected number of columns in v2 changes from 146 to 156. 
...
# 2021-07-09: QDS Visit 2 cleaned with 271 rows and 156 columns.
```
